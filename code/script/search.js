...
document.querySelector('#search-input').addEventListener('input', handleAutocomplete);
...
function handleAutocomplete(event) {
    ...
    // Only trigger autocomplete if the input has at least 2 characters
    if (query.length < 2) {
        return;
    }
  
    // Fetch all Pokémon data to filter names based on the input
    fetch(`https://pokeapi.co/api/v2/pokemon?limit=1025`)
        .then(response => response.json())
        .then(data => {
            const pokemonNames = data.results
                .map(pokemon => pokemon.name)
                .filter(name => name.startsWith(query)); // Only show names that start with the query
...
function updateDivStyle(pokemonType) {
    ...
    const backgroundBorderDiv = document.querySelector('.pokemonBox');
    ...
    backgroundBorderDiv.style.visibility = `visible`;
